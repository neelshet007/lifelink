...
// Immediately emit the donor's current GPS position to the hospital so the
// map updates without waiting for the next watchPosition tick (~5 s).
function emitCurrentPosition(socket, requestId, userId) {
  ...
  const handleAction = async (type) => {
    ...
    try {
      ...
      if (type === 'accept') {
        ...
        // 2. Update local UI immediately — transition to Navigation Mode
        patchActiveEmergency(
          {
        ...
        // 3. Kick off the initial position broadcast so the hospital map
        //    shows the donor's location right away (before next watchPosition tick)
        emitCurrentPosition(socket, activeEmergency.requestId, donorId);
        return;
      }
    ...
    return (
      <div className="fixed bottom-4 right-4 z-[70]">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full border border-red-400/30 bg-red-500/20 px-4 py-3 text-sm font-semibold text-red-50 shadow-[0_18px_48px_rgba(127,29,29,0.3)] backdrop-blur-xl animate-pulse"
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-2">
              <Badge variant={navigationMode ? 'success' : 'saffron'}>
                {navigationMode ? <Navigation className="h-3.5 w-3.5" /> : <Siren className="h-3.5 w-3.5 animate-pulse" />}
                {navigationMode ? 'Navigation Mode — En Route' : 'Incoming Emergency'}
              </Badge>
        ...
        <CardContent className="grid gap-4 pt-6 lg:grid-cols-[1.1fr,0.9fr]">
          <div className="space-y-4">
            ...
            {activeEmergency.responseStatus !== 'accepted' ? (
              ...
              <div className="rounded-[1rem] border border-emerald-400/20 bg-emerald-500/10 p-4 text-sm text-emerald-50 flex items-start gap-3">
                <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />
                Response locked in. Navigation mode is active — your live position is being streamed to the hospital every 5 seconds.
              </div>
            )}
          </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-sm font-semibold text-white">
                  {navigationMode ? 'Streaming position to hospital' : 'Hospital destination map'}
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-[1rem] border border-white/10 bg-white/6 p-4 text-sm text-slate-200">
                ...
                <p className="mt-2 font-semibold text-white">
                  {navigationMode ? '🟢 En route to facility' : '⏳ Awaiting your decision'}
                </p>
              </div>
            </div>
          </div>
